...
## A real life example: improving my diet {.center}

<center>

![Nutrition Facts](images/nutrition_facts.png)
...
![Cereal A](images/wheaties.PNG)

</div>

<div class = 'right'>

![Cereal B](images/frosted_flakes.PNG)
...
## A better data source {.center}

![Instacart Orders](images/orders.PNG)
...
animate(p,
        duration = 60, # 32 weeks * 1 sec/week = 32 seconds
        fps = 15
        )
```

## Starting a 5 day/week gym routine
...
orders_nutrition %>% 
  ...
  ylab("Calories Per Day") + 
  ggtitle("Daily Calories") + 
  annotate("rect", xmin = 13, xmax = 17, ymin = 2000, ymax = 4000, alpha = 0.2, color = "red") + 
  scale_x_continuous(labels = seq(ymd("2019-03-01"), ymd("2019-07-31"), by = "month")) +
  ylim(1000, 5000)
```

## After realizing that I was behind on studying and trying to catch up
...
orders_nutrition %>% 
  ...
  ylab("Calories Per Day") + 
  ggtitle("Daily Calories") + 
  annotate("rect", xmin = 20, xmax = 25, ymin = 2300, ymax = 3300, alpha = 0.2,color = "red") + 
  scale_x_continuous(labels = seq(ymd("2019-03-01"), ymd("2019-07-31"), by = "month")) +
  ylim(1000, 5000)
```
...
orders_nutrition %>% 
  ...
  ylab("Calories Per Day") + 
  ggtitle("Daily Calories") + 
  annotate("rect", xmin = 26, xmax = 30, ymin = 1300, ymax = 3000, alpha = 0.2,color = "red") + 
  scale_x_continuous(labels = seq(ymd("2019-03-01"), ymd("2019-07-31"), by = "month")) +
  ylim(1000, 5000)
```
...
stats_by_day <- orders_nutrition %>% 
  mutate(month = month(date)) %>% 
  filter(month < 8, month > 2) %>% 
  group_by(month) %>% 
  summarise_at(measure_columns, ~sum(.x, na.rm = T)/unique(days_in_month(date))) %>% 
  gather(stat, value, -month)
...
stats_by_day %>% 
  ...
  ggplot(aes(month, value, color = stat)) + 
  geom_line() + 
...
stats_by_day %>% 
  filter(month >2, stat != "sat_fat_g", stat != "trans_fat_g", stat != "sodium_mg") %>% 
  left_join(nutrition_benchmarks, by = "stat") %>% 
  mutate(percent = value/benchmark,
         stat = case_when(
    stat == "calories" ~ "Calories",
    stat == "cholesterol_mg" ~ "Cholesterol (mg)",
    stat == "dietary_fiber_g" ~ "Dietar Fiber (g)",
  ...
  ggplot(aes(stat, percent)) + 
  geom_bar(stat = "identity", position = position_dodge2(), width = 0.2) + 
  coord_flip() + 
  xlab("") + 
  ylab("Percentage of Benchmark") + 
  scale_y_continuous(labels = percent, limits = c(0, 2.5)) +
  transition_time(month) + 
  theme_minimal() + 
  labs(title = "Month: {frame_time}") +
  geom_hline(yintercept = 1.00, color = "red", size = 1.2, alpha = 0.5) + 
  annotate(geom = "label", x = "Total Fat (g)", y = 1.1, color = "red",alpha = 0.8, label = "100% Percent of \nDaily Value")
...
## Exploratory analysis requires speed and iteration {.center}

![Hadley Wickham's Data Science Cycle](images/hadley_data_lifecycle.png)

## Persuading an audience is a *very* different challenge

*How to Win People to Your Way of Thinking - Dale Carnegie*
...
## Data visualization communicates your brand {.center}

```{r fig.width= 5, fig.height=5}
iris %>% 
  ggplot(aes(Sepal.Width, Sepal.Length, color = Species)) + 
  geom_point() +
  ggtitle("Default Graphics")
```
...
## Actuarial work should be easy to read
...
3)  Focus on speed and flexibility (Python or R are a must)
