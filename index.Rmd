...
* When the model fits well lots of empty space in the northwest and southeast quadrants of the display
    * Lots of white space == loss of visual opportunity
    * Signal to the modeler that this vis may not work as well as another.
* Good to add a line for y = x. 
* Consider plotting residual (= actual - expected) on the y-axis
    * Residual plots are broadly similar to ratio of actual and expected -> absolute vs. relative difference
    * Upside of residuals is that they are easier to interpret within the context of a statistical model. We can also standardize them (scale by the sample variance) and make other adjustments to inspect how they compare to underyling statistical assumptions (mostly relevant for GLMs).
* Boxplots may work better with categories on the x-axis. Reading differences left -> right may be easier than top -> bottom
...
readmissions %>% 
  filter(State %in% c("IL", "MA", "NC", "NY", "PA")) %>% 
  mutate(ave = `Expected Readmission Rate`/`Predicted Readmission Rate`) %>% 
  ggplot(aes(`Expected Readmission Rate`, `Predicted Readmission Rate`, color = State)) + 
  geom_point() + 
  theme_soa() + 
  labs(
    title = "Actual vs Expected Readmission Ratio"
    , y = "Predicted"
    , subtitle = "Based on Five Thirty Eight's Theme from the 'ggthemes' package"
    , caption = "Copyright 2019 YourBrandName \nData Source: HealthData.gov") + 
    NULL
  # annotation_custom(logo, ymin = 8.5, ymax = 9, xmin = 4, xmax = 4.5) 
```
